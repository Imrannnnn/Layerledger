...
export function Inp({label,type="text",value,onChange,placeholder,hint,disabled,style={}}){return<div style={{marginBottom:13}}>{label&&<label style={{display:"block",fontSize:11.5,color:"var(--muted)",fontWeight:500,marginBottom:4,textTransform:"uppercase",letterSpacing:0.5}}>{label}</label>}<input type={type} value={value} onChange={e=>onChange(type==="number"?+e.target.value:e.target.value)} placeholder={placeholder} disabled={disabled} style={{...iSt,...style}}/>{hint&&<div style={{fontSize:11,color:"var(--muted)",marginTop:3}}>{hint}</div>}</div>}
