...
export function Inp({label,type="text",value,onChange,placeholder,hint,disabled,step,min,max,style={},...props}){
  const emailProps = type === "email" ? { autoCapitalize: "none", autoCorrect: "off", spellCheck: false } : {}
  return <div style={{marginBottom:13}}>{label&&<label style={{display:"block",fontSize:11.5,color:"var(--muted)",fontWeight:500,marginBottom:4,textTransform:"uppercase",letterSpacing:0.5}}>{label}</label>}<input type={type} value={value ?? ""} onChange={e=>onChange(e.target.value)} placeholder={placeholder} disabled={disabled} step={step !== undefined ? step : (type === "number" ? "any" : undefined)} min={min} max={max} style={{...iSt,...style}} {...emailProps} {...props}/>{hint&&<div style={{fontSize:11,color:"var(--muted)",marginTop:3}}>{hint}</div>}</div>
}
export function Sel({label,value,onChange,options=[],hint,style={}}){return<div style={{marginBottom:13}}>{label&&<label style={{display:"block",fontSize:11.5,color:"var(--muted)",fontWeight:500,marginBottom:4,textTransform:"uppercase",letterSpacing:0.5}}>{label}</label>}<select value={value} onChange={e=>onChange(e.target.value)} style={{...iSt,cursor:"pointer",...style}}>{options.map(o=><option key={o.value||o} value={o.value||o}>{o.label||o}</option>)}</select>{hint&&<div style={{fontSize:11,color:"var(--muted)",marginTop:3}}>{hint}</div>}</div>}
...
export function Badge({children,color="gold",style={}}){const c={gold:["#FFF3DC","#7A5200"],green:["#E1F5EE","#085041"],blue:["#E6F1FB","#0C447C"],red:["#FCEBEB","#501313"],gray:["#F1EFE8","#44403C"],purple:["#F3E8FF","#5B21B6"]}[color]||["#FFF3DC","#7A5200"];return<span style={{background:c[0],color:c[1],fontSize:11,fontWeight:600,padding:"3px 8px",borderRadius:20,letterSpacing:0.3,display:"inline-block",...style}}>{children}</span>}
...
export function TH({cols}){return<thead><tr style={{background:"#EDE5D6"}}>{cols.map((c, idx)=><th key={typeof c === "string" || typeof c === "number" ? `${c}-${idx}` : idx} style={{padding:"8px 10px",textAlign:"left",fontSize:10,textTransform:"uppercase",letterSpacing:0.8,color:"var(--muted)",fontWeight:500,whiteSpace:"nowrap"}}>{c}</th>)}</tr></thead>}
...
export function FullPageLoader({ message = "Signing in & loading your bakery records..." }) {
  return (
    <div style={{
      display: "flex",
      ...
      <div style={{
        ...
        maxWidth: 380,
        width: "100%",
        boxShadow: "0 8px 24px rgba(41,22,8,0.06)",
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        gap: 16
        ...
        <div style={{
          ...
          <Cake size={28} color="var(--gold, #C8912A)" />
        </div>
        <div>
          <div style={{
          ...
          <div style={{ fontSize: 13.5, color: "var(--muted, #8C6E52)", lineHeight: 1.5 }}>
            {message}
          </div>
        </div>
        <div style={{
          width: 30,
          height: 30,
          border: "3px solid rgba(200,145,42,0.2)",
          borderTopColor: "var(--gold, #C8912A)",
          borderRadius: "50%",
