  ...
  const dl=()=>{
    ...
    *{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;color:#291608;padding:40px;max-width:780px;margin:0 auto}
    ...
    ${inventory.map(item=>{
      ...
      return`<tr><td>${item.name}</td><td>${unit}</td><td class="right">${opening} ${unit}</td><td class="right" style="color:#1D9E75">see receipts</td><td class="right" style="color:#B03A2E">−${used.toFixed(2)} ${unit}</td><td class="right"><strong>${closing} ${unit}</strong></td><td class="right">₦${Math.round(cost).toLocaleString()}</td></tr>`
    }).join("")}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:16}}>
      {[{l:"Total Purchased",v:fmt(totalPurchased),s:"from receipts this month",c:"#357A52"},
        ...
        {l:"Closing Stock Value",v:fmt(inventory.reduce((s,i)=>{
          ...
          const cost = f ? f.cost : i.cost
          return s + i.stock * cost
        },0)),s:"current live value",c:"var(--gold)"}
